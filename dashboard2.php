...
?></td></tr>
<tr><td>Total Sales This Year</td><td style="padding-left: 40%;"><?php 

$sql = "SELECT count(order_id) count from orders where Year(date)='$yearname'";
$result = $conn->query($sql);
if($row = $result->fetch_assoc())
{
echo $row['count'];
...
<?php 
$sql = "SELECT count(order_id) count FROM orders where order_status!='Completed' and Year(date)='$yearname'";
$result = $conn->query($sql);
if($row = $result->fetch_assoc())
{
echo $row['count'];
...
<tr><td colspan='2'><div id='d'><button><a href="index.php?dashboard">Month</a></div></button>
</td></tr>
</table>
...
<div class="graph-info">
 ...
 -->
	 ...
	 <canvas id="mycanvas"></canvas></div>
	  ...
	  <script src="/mokshaa/js/app.js"></script>
